...
public class Utility {
	...
	public static String rpaging(int total, int nowPage, int recordPerPage, String col, String word, String url,
		...
		for (int i = startPage; i <= endPage; i++) {
			...
			if (nPage == i) {
				str.append("<li class='active'><a href=#>" + i + "</a></li>");//페이지 선택시 마다 해당페이지에 대한 내용이 보여짐
			} else {
				str.append("<li><a href='" + url + "?col=" + col + "&word=" + word + "&nowPage=" + nowPage + fk
						+ fno + "&nPage=" + i + "'>" + i + "</A></li>");
			}
	...
	//-----------------------------------------------------------------------------------------
	//imgup 댓글갯수
	/*	  public static int ircount(int imgno,IreplyInter rinter){ 
	          return rinter.rcount(imgno);
